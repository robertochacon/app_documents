...
if($_SERVER['REQUEST_METHOD'] == 'GET' && $_GET['opc'] == 'all'){
   ...
   $template .= ' <table id="types_table" class="table table-striped table-bordered" style="width:100%">';
   ...
   $template .= ' <tbody id="datos_tipos">';
    ...
    foreach($datos as $item){
        ...
        $template .= '<td><img src="./assets/img/folder.png" width=""></td>';
        $template .= '<td>'.$item['id'].'</td>';
        $template .= '<td>'.$item['name'].'</td>';
        $template .= '<td>'.$status.'</td>';
        $template .= '<td><button class="btn btn-info text-white" onclick="edit_type('.$item['id'].')">Editar</button><button class="btn btn-danger text-white ml-2" onclick="delete_type('.$item["id"].')">Eliminar</button></td>';
        $template .= '</tr>';
    }
    ...
    if($opc == 'insert'){
        ...
        $sql = "INSERT INTO types (name,status)
        VALUES ('".$name."',$status)";
        ...
        $sql = "UPDATE types SET name = '".$name."', status = $status WHERE id = $id";
        $result = $conexion->ejecutar($sql);
        if ($result) {
            return true;
        }else{
